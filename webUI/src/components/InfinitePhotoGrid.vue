<template>
  <div>
    ...
    <div v-if="loading && photos.length === 0" class="loading-skeleton">
      <div v-for="n in 6" :key="n" class="skeleton-row">
        <v-skeleton-loader 
          ...
          :key="m" 
          type="image" 
          :style="{ width: getRandomSkeletonWidth(), height: '200px', marginRight: '4px' }"
        ></v-skeleton-loader>
      </div>
    </div>
    ...
    <v-dialog
      ...
      scrollable
    >
      <v-card v-if="currentPhoto" class="fullscreen-card">
        <!-- Fixed top bar with close and download buttons -->
        <v-app-bar
          color="rgba(0, 0, 0, 0.8)"
          density="compact"
          flat
          class="fullscreen-app-bar"
        >
          <v-spacer></v-spacer>
...
const processedPhotos = computed(() => {
  return props.photos.map((photo, index) => {
    ...
    if (photo.width && photo.height) {
      ...
      aspectRatio = Math.random() > 0.7 ? (Math.random() > 0.5 ? 4/3 : 3/4) : (Math.random() > 0.3 ? 16/9 : 1)
    }
...
const justifiedRows = computed(() => {
  ...
  for (let i = 0; i < processedPhotos.value.length; i++) {
    ...
    if (currentRow.length > 0 && (currentRowWidth + photoWidth + (currentRow.length * spacing)) > availableWidth) {
...
const getRandomSkeletonWidth = () => `${Math.floor(Math.random() * 200) + 150}px`
...
const getImageUrl = (photo) => {
  ...
  if (photo.thumbnailFileName) {
    return `${import.meta.env.VITE_CLOUDFRONT_DOMAIN}/${photo.thumbnailFileName}`
  }
...
const downloadPhoto = async () => {
  ...
  try {
    ...
    link.download = currentPhoto.value.fileName || `photo-${currentPhoto.value.PK || Date.now()}.jpg`
...
.fullscreen-content {
  ...
  display: flex;
  flex-direction: column;
  padding-top: 48px; /* Account for fixed app bar */
  padding-bottom: 60px; /* Account for fixed bottom controls */
}
...
.fullscreen-image {
  width: 100%;
  height: 100%;
  max-height: calc(100vh - 108px); /* Subtract app bar and bottom controls height */
}
...
.photo-details {
  position: absolute;
  top: 60px; /* Below the fixed app bar */
  right: 0;
  max-width: 300px;
  z-index: 3;
}
...
@media (max-width: 768px) {
  ...
  .photo-details {
    position: fixed;
    top: auto;
    bottom: 60px; /* Above bottom controls */
    left: 0;
    right: 0;
    max-width: none;
    z-index: 999;
